...
public class AsteroidControlSystem implements IEntityProcessingService, IMovableProcessingService {
    ...
    public void process(GameData gameData, World world) {
        ...
        for (Entity entity : this.world.getEntities(Asteroid.class)) {
            splitAsteroidIfNeeded((Asteroid) entity);
            moveEntity(entity);
            removeOutOfWindowEntity(entity);
        }
    }
    ...
    public void moveEntity(Entity entity) {
        if (entity instanceof Asteroid) {
            ...
            entity.setRotation(asteroid.getRotation() + asteroid.getRotationSpeed());

            entity.setXCoordinate(asteroid.getXCoordinate() + changeX * asteroid.getMovingSpeed());
            entity.setYCoordinate(asteroid.getYCoordinate() - changeY * asteroid.getMovingSpeed());
        }
    }
    ...
    public Asteroid createAsteroidAtRandomWindowSide() {
        ...
        Asteroid asteroid = new Asteroid(4, 20, 4, shapeCoordinates);
    ...
    public void splitAsteroidIfNeeded(Asteroid asteroid) {
        if (asteroid.getAsteroidSize() > 1 && asteroid.getLivePoints() <= 0) {
    ...
    private void splitAsteroid(Asteroid oldAsteroid) {
        ...
        double movingSpeed = getAsteroidSizeBasedMovingSpeed(newSize);
        double rotationSpeed = getMovingSpeedBasedRotationSpeed(newSize);
        ...
        Asteroid newAsteroid1 = createNewSplitAsteroid(oldAsteroid, newSize, movingSpeed, rotationSpeed, 1, newCoordinates);
        Asteroid newAsteroid2 = createNewSplitAsteroid(oldAsteroid, newSize, movingSpeed, rotationSpeed, -1, newCoordinates);
     ...
     */
    public Asteroid createNewSplitAsteroid(Asteroid oldAsteroid, int newSize, double movingSpeed, double rotationSpeed, int offsetDirection, double[] newCoordinates) {
        // Create new asteroid
        Asteroid newAsteroid = new Asteroid(newSize, movingSpeed, 4, newCoordinates);
     ...
     */
    private double getAsteroidSizeBasedMovingSpeed(int size) {
        return (0.5 / size) * 100;
    }

    /**
     * Returns the rotation speed based on the given moving speed.
     *
     * @param movingSpeed the moving speed of the asteroid
     * @return the rotation speed of the asteroid
     */
    private double getMovingSpeedBasedRotationSpeed(double movingSpeed) {
        return movingSpeed * 2;
    }
}
